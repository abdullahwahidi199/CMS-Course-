...
export default function UserManagementPage() {
  ...
  const saveUser = async (event) => {
    ...
    try {
      ...
      setError(err.response?.data ? JSON.stringify(err.response.data) : "Could not save user.");
    }
  };
  ...
  const bulk = async (rows, actionName) => {
    await apiPost("/v1/users/bulk-action/", { user_ids: rows.map((row) => row.id), action: actionName });
  ...
  const columns = [
    ...
    { key: "name", label: "Name", render: (row) => `${row.first_name || ""} ${row.last_name || ""}`.trim() || row.username },
    { key: "email", label: "Email" },
    { key: "phone", label: "Phone" },
    { key: "role_name", label: "Role" },
    { key: "is_active", label: "Status", render: (row) => (row.is_active && !row.is_deactivated ? "Active" : "Inactive") },
    { key: "last_login", label: "Last Login", render: (row) => (row.last_login ? new Date(row.last_login).toLocaleString() : "Never") },
  ];

  return (
    <div className="space-y-6">
      <PageHeader title="User Management" description="Create users, assign roles, control access, and manage account status." />
      ...
      <form className="rounded-md bg-white p-4 shadow-sm" onSubmit={saveUser}>
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Field label="Role">
            <select className={inputClass()} value={form.role || ""} onChange={(event) => setForm({ ...form, role: event.target.value })}>
              ...
              {roles.results.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}
            </select>
          </Field>
        ...
        <div className="mt-4 flex gap-2">
          <button className="inline-flex items-center gap-2 rounded-md bg-cyan-700 px-4 py-2 text-sm font-semibold text-white"><Save size={16} /> {editingId ? "Update User" : "Create User"}</button>
          <button type="button" className="rounded-md border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700" onClick={() => { setForm(emptyUser); setEditingId(null); }}>Reset</button>
        </div>
      </form>
      ...
      <DataTable
        ...
        bulkActions={[
          ...
          { label: "Delete", onClick: (rows) => window.confirm(`Delete ${rows.length} users?`) && bulk(rows, "delete") },
        ]}
        actions={(row) => [
          { label: "Edit", onClick: () => editUser(row) },
          { label: "Role", onClick: () => assignRole(row) },
          { label: "Reset Password", onClick: () => resetPassword(row) },
          { label: row.is_active ? "Deactivate" : "Activate", onClick: () => action(row, row.is_active ? "deactivate" : "activate", "User status updated.") },
          { label: "Delete", onClick: async () => { if (window.confirm(`Delete ${row.username}?`)) { await apiDelete(`/v1/users/${row.id}/`); await users.refetch(); } } },
        ]}
      />
    </div>
  );
