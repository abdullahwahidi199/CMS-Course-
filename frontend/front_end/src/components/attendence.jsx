...
export default function Attendance() {
  ...
  const fetchStudents = async () => {
    if (!classid) return; // ✅ IMPORTANT FIX

    try {
      const response = await instance.get(`/students/by-class/${classid}/`);
    ...
    setAttendance(
      attendance.map((item) =>
        item.student_id === studentId
          ? { ...item, is_present: !item.is_present }
          : item,
      ),
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const parsedTokens = JSON.parse(savedTokens);
      const response = await instance.post(`/attendance/mark/${classid}/`, {
        date,
        attendance,
      });
    } catch (error) {
  ...
  return (
    <div className="p-6 space-y-8 bg-gray-50 min-h-screen">
      <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
        <CheckCircle size={28} className="text-blue-600" /> Mark Class
        Attendance
      </h1>
