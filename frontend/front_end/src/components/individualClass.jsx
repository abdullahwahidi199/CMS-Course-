...
function IndividaulClass() {
  ...
  const handleDelete = async () => {
    try {
      await instance.delete(`/classes/${id}/`);
      navigate(-1);
    } catch (error) {
      console.error(error.message);
    }
  ...
  const updateClass = async () => {
    ...
    try {
      await instance.patch(`/classes/${id}/`, payload);
      navigate(-1);
    } catch (error) {
          ...
          <div className="rounded shadow-lg p-15 flex flex-col">
            <div className="flex justify-between items-center">
              ...
              <div className="space-y-3 text-sm md:text-base">
                ...
                <h2>{cls.end_time}</h2>
                {cls.roomOfClass_details && (
                  <h2>{cls.roomOfClass_details.name}</h2>
                )}
              </div>
            </div>
            <div className="inline-flex gap-2 space-x-4">
              <button
                onClick={() =>
                  window.confirm("Are you sure you want to delete the class?")
                    ? handleDelete()
