...
function Classes() {
  ...
  const handleAssignTeachers = async (classID) => {
    ...
    try {
      const parsedTokens = JSON.parse(savedTokens);
      const classRes = await instance.get(`/classes/${classID}/`);
      const classData = classRes.data;
      const currentTeacherIDs = classData.teachers;
      const updatedTeacherIDs = [
        ...new Set([...currentTeacherIDs, teacherIDToADD]),
      ];

      const response = await instance.patch(`/classes/${classID}/`, {
        teachers: updatedTeacherIDs,
      });

      const updatedClassWithTeachers = response.data;
      setClasses((prevClasses) =>
        prevClasses.map((cls) =>
          cls.id === classID ? updatedClassWithTeachers : cls,
        ),
      );
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="teachers">
                ...
                <div
                  style={{ display: teacherFormDisplay ? "block" : "none" }}
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        window.confirm(
                          "Are u sure u want to remove the teacher from the class?",
                        )
                          ? handleDeleteTeachers(classItem.id)
                          : ""
                      }
                      className="bg-green-600 text-white px-3 py-1 rounded"
                    >
                    ...
                    <tbody key={student.id}>
                      <tr className="border-t">
                        ...
                        <td className="text-blue-600 hover:underline -">
                          <Link
                            to={`/admin/dashboard/student/${student.id}`}
          ...
          <div className="bg-white p-6 rounded-lg w-full max-w-xl shadow-lg">
            ...
            >
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button
                  type="button"
                  onClick={handleAddClassFormDisplay}
