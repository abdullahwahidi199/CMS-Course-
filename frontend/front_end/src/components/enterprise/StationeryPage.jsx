...
export default function StationeryPage() {
  ...
  const lowStock = items.results.filter(
  ...
  const completeSale = async (event) => {
    ...
    try {
      ...
      if (response) {
        ...
        setError("Sale completed but receipt data missing.");
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Stock levels, sales, receipts, low-stock alerts, and purchase history."
      />
      {message ? (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Item Name">
              <input
                ...
                onChange={(event) =>
                  setItemForm({ ...itemForm, item_name: event.target.value })
                }
              />
            </Field>
            ...
            <Field label="Barcode">
              <input
                ...
                onChange={(event) =>
                  setItemForm({ ...itemForm, barcode: event.target.value })
                }
              />
            </Field>
            <Field label="Cost Price">
              <input
                ...
                onChange={(event) =>
                  setItemForm({ ...itemForm, cost_price: event.target.value })
                }
              />
            </Field>
        ...
        >
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Action">
              <select
                className={inputClass()}
                value={stockMove.type}
                onChange={(event) =>
                  setStockMove({ ...stockMove, type: event.target.value })
                }
              >
            ...
            <div className="grid max-h-64 gap-2 overflow-auto md:grid-cols-2">
              {items.results.map((item) => (
                <button
                  type="button"
                  key={item.id}
                  className="rounded-md border border-gray-200 p-3 text-left text-sm hover:border-cyan-600"
                ...
                >
                  ...
                  <div className="text-gray-500">
                    {item.barcode || item.sku} - stock {item.quantity}
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="max-h-64 overflow-auto rounded-md border border-gray-100">
              {cart.length === 0 ? (
                <div className="p-4 text-sm text-gray-500">
                  No sale items selected.
                </div>
              ) : (
                cart.map((line, index) => (
                  <div
                    key={line.item}
                    className="grid grid-cols-[1fr_80px_90px_40px] items-center gap-2 border-b border-gray-100 p-2 text-sm"
                  >
                    ...
                    <button
                      type="button"
                      className="rounded-md border border-gray-200 py-2"
                      onClick={() =>
                        setCart((current) =>
                          current.filter((item) => item.item !== line.item),
                        )
                      }
                    >
                      x
        ...
        bulkActions={[
          {
            label: "Delete",
            onClick: (rows) =>
              window.confirm(`Delete ${rows.length} items?`) &&
              Promise.all(rows.map(deleteItem)),
          },
        ]}
        actions={(row) => [
          ...
          {
            label: "Barcode",
            onClick: () => setMessage(`Barcode: ${row.barcode || row.sku}`),
          },
